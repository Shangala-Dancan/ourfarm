...
export default function Dashboard() {
  ...
  const stats = [
    { label: "Total Herd", value: animals.length.toString(), icon: CowIcon, sub: `${animals.filter((a) => a.status === "active").length} active` },
    { label: "Today's Milk", value: `${todayMilk.toFixed(1)}L`, icon: Milk, sub: `${milkRecords.length} total records` },
    { label: "Pending Calving", value: pendingCalving.toString(), icon: AlertTriangle, sub: `${recentMovements} movements this week` },
    { label: "Monthly Revenue", value: `Kshs ${monthlyRevenue.toFixed(0)}`, icon: DollarSign, sub: `Kshs ${monthlyExpenses.toFixed(0)} expenses` },
  ];
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <Button variant="ghost" size="sm" onClick={() => navigate("/milk")}>View All <ArrowRight className="h-4 w-4 ml-1" /></Button>
          </CardHeader>
          <CardContent className="h-[220px]">
            {milkTrend.length === 0 ? <p className="text-sm text-muted-foreground">No production data yet.</p> : (
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <Button variant="ghost" size="sm" onClick={() => navigate("/finances")}>View All <ArrowRight className="h-4 w-4 ml-1" /></Button>
          </CardHeader>
          <CardContent className="h-[220px]">
            {financeTrend.length === 0 ? <p className="text-sm text-muted-foreground">No financial data yet.</p> : (
              <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          ...
          <CardContent>
            {recentActivity.length === 0 ? (
              <p className="text-sm text-muted-foreground">No activity yet. Start by adding animals to your herd.</p>
            ) : (
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-2 gap-2">
            <Button variant="outline" size="sm" onClick={() => navigate("/herd")}><Plus className="h-4 w-4 mr-1" /> Add Animal</Button>
            <Button variant="outline" size="sm" onClick={() => navigate("/milk")}><Plus className="h-4 w-4 mr-1" /> Log Milk</Button>
            <Button variant="outline" size="sm" onClick={() => navigate("/movements")}><Plus className="h-4 w-4 mr-1" /> Log Movement</Button>
            <Button variant="outline" size="sm" onClick={() => navigate("/finances")}><Plus className="h-4 w-4 mr-1" /> Add Transaction</Button>
          </CardContent>
        </Card>
      </div>
    </div>
